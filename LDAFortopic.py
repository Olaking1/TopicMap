...
class loadFiles(object):
                ...
                if os.path.isfile(file_path):
                    ...
                    try:
                        ...
                        for i in range(len(string.punctuation)):
                            str += ' '
                        table = content.maketrans(string.punctuation, str)
                        content = content.translate(table)
                        # content = ' '.join(filter(lambda x: x, content.split(' ')))
    ...
    def delete_punctuation(self, content):
        str = ''
        for i in range(len(string.punctuation)):
            str += ' '
        table = content.maketrans(string.punctuation, str)
...
def convert_doc_to_wordlist(str_doc, cut_all):
    ...
    jieba.load_userdict('/Users/liyue/Documents/datasets/use_words')
...
if __name__ == '__main__':
    path_doc_root = '/Users/liyue/Documents/datasets/csdn_after'  # 根目录 即存放按类分类好的文本集
    path_tmp = '/Users/liyue/Documents/datasets/csdn_tmp'  # 存放中间结果的位置
    ...
    if not os.path.exists(path_tmp_lda):
        ...
        lda_model = models.LdaModel(corpus=corpus_tfidf_total, id2word=dictionary, num_topics=7, alpha='auto',
                                    eval_every=1)
        # 将lda模型存储到磁盘上
    ...
    topic = lda_model.print_topics(7, 15)
    print(topic)
    index = similarities.docsim.Similarity(output_prefix='/Users/liyue/Documents/datasets/csdn_tmp', corpus=corpus,
                                           num_features=len(dictionary))
    filePath = os.path.join(path_tmp,'similaritiesFiles')
    file = open(filePath, 'w')#若文件存在，则先清空再打开
