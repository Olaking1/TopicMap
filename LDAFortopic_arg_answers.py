...
def convert_doc_to_wordlist(str_doc, cut_all):
    ...
    for part in sent_list:
        ...
        for i in list_all:
            # 跑复旦数据集要加上'eng'
            if(i.flag not in ['x', 'm', 'u', 'p', 'f', 'c', 'ul', 'uj', 'e']):
...
def rm_tokens(words):  # 去掉一些停用词和数字
    ...
    for i in range(words_list.__len__())[::-1]:
        ...
        if words_list[i] in stop_words:  # 去除停用词
            ...
            words_list.pop(i)
        elif words_list[i].strip() in punctuation:
            words_list.pop(i)
...
def path_compare():
    ...
    add_labels(rect2)
    plt.subplots_adjust(left=0.08, bottom=0.20, right=0.98, top=0.80)
...
def degree_distribution():
    ...
    X3.sort()
    X3_2 = range(1, 670)
    Y3_2 = 100 * scipy.power(X3_2, -1.3)
    ax3.set_title('Degree distributions of TMC topic map')
...
def effeciency_compute():
    ...
    for i, node in zip(range(len(lda_higher_degree_nodes)),lda_higher_degree_nodes):
        ...
        if((len(lda_higher_degree_nodes)-i) > 5 and (lda_init_eff - eff) >= 0.002):
            ...
            lda_init_eff = eff
            print("节点编号:%d, 文档名：%s"%(node, filenames[node]))
...
if __name__ == '__main__':
    ...
    if not os.path.exists(path_dictionary):
        ...
        init_catg = ['mybatis','Docker','mysql','linux','Hadoop','HBase','mongoDB']
    ...
    if not os.path.exists(path_temp_topicmaps):
        ...
        for item in doc_index:
            lda_sim_between_doc.append(item)
        # 对相似度进行排序
        init_list = []
        for i in range(0, len(lda_sim_between_doc)):
            for j in range(i + 1, len(lda_sim_between_doc[i])):
                if(lda_sim_between_doc[i][j] < 0.99):
                    init_list.append(lda_sim_between_doc[i][j])
        init_list.sort(reverse=True)
        print("第22405个相似度为%f" % init_list[22405])
        ...
        for i, similars in zip(range(len(filenames)), doc_index):
            ...
            for j in range(len(similars)):
                # 数据集2：:22300
                if(graph.number_of_edges() <= 22405 and similars[j] >=0.60 and similars[j] < 0.99):
                    graph.add_node(i, topic=fileTopicI[0][0])
                    graph.add_edge(i,j, weight=similars[j])
        g = nx.connected_component_subgraphs(graph)
        max_nodes = 0
        max_lda_subgraph = nx.Graph()
        for item in g:
            if (item.number_of_nodes() < 50):
                print(item.nodes())
            if (item.number_of_nodes() > max_nodes):
                max_nodes = item.number_of_nodes()
                max_lda_subgraph = item
        graph = max_lda_subgraph
    ...
    if not os.path.exists(lsi_path_temp_topicmaps):
        ...
        while (not nx.is_connected(lsi_graph)) and np.max(sim_between_doc) > 0.60:
            ...
            # count = count+1
        print("============循环结束==============")
        print("连通子图数为：%d" % nx.number_connected_components(lsi_graph))
        print("连通子图的节点个数分别为:")
